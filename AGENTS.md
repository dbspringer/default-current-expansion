...
That invariant depends on how ownership is *acquired*, which is easy to get wrong: claim only when the call actually flips the filter from off to on. Recording ownership on every apply looks equivalent and is not — a player who had the filter ticked before installing would have it claimed on the first AH visit and deleted on the next toggle-off. The consequence of the conservative rule is that filters left on by a pre-ownership version are treated as the player's, so disabling an option will not release them until the player unticks once; that is the safer direction to be wrong in, and it self-corrects.
